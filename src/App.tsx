          ...
          >
            ...
            <div className="text-white/75 leading-relaxed space-y-3 text-sm">
              {content.split('\n').map((line, i) => {
                ...
                return <motion.p key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.01 }} className="text-white/70 leading-relaxed">{line}</motion.p>;
              })}
            </div>
            <div className="pt-4 flex justify-end border-t border-white/8">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }} onClick={onClose} className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-black text-sm relative overflow-hidden group" style={{ background: 'linear-gradient(135deg, #c8102e, #a50d25)' }}>
  ...
  return (
    <div className="flip-card-back flip-card-face" style={{ '--accent': color } as React.CSSProperties}>
      ...
      <div className="relative shrink-0 flex items-center justify-between gap-2 px-4 pt-3 pb-2.5"
        ...
        {/* ✅ FIX: e.stopPropagation() so the flip doesn't re-trigger */}
        <button
    ...
    <div className="relative">
      ...
      <div className="flex items-center justify-between px-2 mt-4">
        <motion.button onClick={() => goTo(activeIndex - 1)} disabled={activeIndex === 0}
          whileTap={{ scale: 0.9 }}
          className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-black border border-white/10 bg-white/5 disabled:opacity-25 transition-opacity">
...
const ChangePackageModal = ({
  ...
  isOpen: boolean; onClose: () => void; currentPackageId: string; packages: Package[]; vipPackage: Package; duoPackage: Package;
  equipmentPackages: Package[]; businessPackage: Package; transportPackage: Package; onSelect: (p: Package) => void; lang: Language;
}) => {
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ type: 'spring', stiffness: 300 }} className="space-y-5">
              ...
              <div className="relative">
                ...
                <motion.div className={`rounded-lg border-2 border-dashed py-3 px-3 flex flex-col items-center gap-1 transition-colors ${formData.paymentProof ? 'border-green-500/50 bg-green-500/5' : 'border-white/10 bg-white/5'}`} whileHover={{ scale: 1.02 }}>
                  {formData.paymentProof ? (
                    <><div className="w-8 h-8 rounded-full bg-green-500/20 flex items-center justify-center"><Check size={16} className="text-green-400" /></div><span className="text-xs font-black text-green-400">הקובץ הועלה בהצלחה</span></>
                  ) : (
                    <><Upload size={18} className="text-white/40" /><span className="text-xs font-black text-white/60">לחץ להעלאת צילום מסך</span><span className="text-[9px] text-white/30">PNG, JPG או JPEG</span></>
                  )}
                </motion.div>
              </div>
            </div>
      ...
      <div className="flex gap-2">
        ...
        <motion.button onClick={onSubmit} disabled={!paymentMethod || !formData.paymentProof || loading}
          ...
          <span className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
          <span className="relative">
            {loading ? (<span className="flex items-center justify-center gap-2"><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />שולח...</span>) : 'שלח הזמנה'}
          </span>
        </motion.button>
      </div>
    </motion.div>
...
// ============================================================
// SECTION HEADER COMPONENT — for consistent animated headers
// ============================================================
const SectionHeader = ({ badge, badgeIcon, badgeColor, title, titleAccent, accentColor, description }: {
  badge: string; badgeIcon: React.ReactNode; badgeColor: string; title: string; titleAccent: string; accentColor: string; description: string;
}) => (
  <motion.div
  ...
  >
    ...
    >
      <span style={{ color: badgeColor }}>{badgeIcon}</span>
      <span className="text-xs font-black tracking-wider" style={{ color: badgeColor }}>{badge}</span>
    </motion.div>
  ...
  return (
    <div className="min-h-screen text-white" style={{ background: 'linear-gradient(180deg, #06060a 0%, #0a0508 40%, #06060a 100%)' }}>
      <style>{`
        ...
        .fcb-scroll { flex: 1 1 0; min-height: 0; overflow-y: auto; overflow-x: hidden; }
          ...
          {view === 'home' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className="space-y-24">
              ...
              <section id="how-it-works" className="space-y-14">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-2xl mx-auto">
                  <div className="flex items-center justify-between gap-4 p-5 rounded-2xl flex-wrap"
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-brand-red/15 border border-brand-red/25 flex items-center justify-center"><Zap size={18} className="text-brand-red" /></div>
                      <div><p className="text-sm font-black text-white">מוכנים להתחיל?</p><p className="text-[11px] text-white/45">תהליך מהיר, פשוט ומקצועי</p></div>
                    </div>
              ...
              <section id="packages" className="space-y-20">
                {/* Main title */}
                <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center space-y-3">
                  <motion.h2
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                ...
                <div>
                  <SectionHeader
                    badge="חבילות רכב פרטי"
                    badgeIcon={<Car size={13} />}
                    badgeColor="#3b82f6"
                    title="מוכרים"
                    titleAccent="רכב פרטי?"
                    accentColor="#c8102e"
                    description="✨ פרסום מקצועי ברשתות החברתיות · חשיפה לאלפי קונים פוטנציאליים · תוצאות מוכחות תוך 24 שעות"
                  />

                  {/* Desktop */}
                  <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
                    {packages.map((pkg, i) => (
                      <motion.div key={pkg.id} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.12 }} className="h-[500px]">
                  ...
                  <div className="sm:hidden">
                    <MobilePackageSwiper packages={packages} lang={lang} onSelect={handleSelectPackage} />
                  </div>
                </div>
                ...
                <div>
                  <SectionHeader
                    badge="חבילות פרימיום VIP"
                    badgeIcon={<Crown size={13} />}
                    badgeColor="#d4af37"
                    title="מחפשים"
                    titleAccent="יחס VIP?"
                    accentColor="#d4af37"
                    description="👑 חבילות יוקרה לרכבי פרימיום · ליווי אישי מלא 24/7 · עיצוב VIP בלעדי · חשיפה מקסימלית לקהל ממוקד"
                  />
                ...
                <div className="max-w-3xl mx-auto space-y-6">
                  <SectionHeader
                    badge="לסוכנויות ומוכרים מקצועיים"
                    badgeIcon={<Building2 size={13} />}
                    badgeColor="#3b82f6"
                    title="פתרון"
                    titleAccent="לסוכנים ועסקים?"
                    accentColor="#3b82f6"
                    description="🏢 ניהול מלא של הפרסום · חיסכון עצום בעלויות · נציג אישי ייעודי לסוכנות שלך · עד 50 רכבים בחודש"
                  />
                ...
                <div>
                  <SectionHeader
                    badge="ציוד מקצועי ותחבורה"
                    badgeIcon={<Truck size={13} />}
                    badgeColor="#ea580c"
                    title="מוכרים"
                    titleAccent="ציוד מקצועי?"
                    accentColor="#ea580c"
                    description="🚜 ציוד כבד, ציוד קל ותחבורה · חשיפה לקהל מקצועי ממוקד · מפרט טכני מלא ומדויק לכל מודעה"
                  />
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {equipmentPackages.map(pkg => (
                      <div key={pkg.id} className="h-[440px]">
                        <EquipmentPackageCard pkg={pkg} onSelect={handleSelectPackage} />
                      </div>
                    ))}
                      ...
                      <div className="p-6 flex flex-col gap-5 flex-1">
                        <div className="flex items-center justify-between">
                          <div className="w-12 h-12 rounded-xl flex items-center justify-center shadow-lg"
                            style={{ background: `linear-gradient(135deg, ${item.gradFrom}, ${item.gradTo})`, boxShadow: `0 6px 20px ${item.glowColor}` }}>{item.icon}</div>
                  ...
                  <div className="relative z-10 p-10 space-y-10">
                    <div className="text-center space-y-4">
                      <div className="flex items-center justify-center gap-4">
                        ...
                        <div>
                          <div className="text-3xl font-black tracking-tighter"><span className="text-brand-red">YOUGO</span> <span className="text-white">ISRAEL</span></div>
      ...
      <ChangePackageModal
        isOpen={showChangePackage} onClose={() => setShowChangePackage(false)} currentPackageId={selectedPackage?.id || ''}
        packages={packages} vipPackage={vipPackage} duoPackage={duoPackage} equipmentPackages={equipmentPackages}
        businessPackage={businessPackage} transportPackage={transportPackage}
        onSelect={(p) => {
          setSelectedPackage(p);
