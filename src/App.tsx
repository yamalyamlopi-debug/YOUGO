...
// ============================================================
// FLIP CARD COMPONENT - بطاقة قابلة للقلب بشكل احترافي
// ============================================================
const FlipCard = ({ 
  front, 
  back, 
...
// ============================================================
// PACKAGE DETAILS - شرح مفصل لكل باقة
// ============================================================
...
💰 **מחיר:** 749 ₪ (במקום 882 ₪, מחיר השקה)`
  ...
  duo: {
    ...
    content: `🚗🚗 **DUO DEAL – פרסום 2 רכבים** 🚗🚗
...
💰 **מחיר:** 349 ₪ (במקום 598 ₪)`
  ...
  business: {
    ...
    content: `🏢 **BUSINESS – לסוכנויות רכב** 🏢
...
const PackageCard = ({ pkg, lang, onSelect }: PackageCardProps) => {
  ...
  const frontContent = (
    <div
      className="relative w-full h-full rounded-2xl flex flex-col group"
      style={{
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-grow gap-3 h-full">
        ...
        <div className="flex gap-2 mt-2">
          ...
          <div
            className="px-2 py-2.5 rounded-lg font-black text-xs bg-white/5 border border-white/10 hover:bg-white/10 transition-all cursor-pointer"
        ...
        <div className="text-center text-[8px] text-white/30 mt-1">
          👆 לחץ על כרטיס לפרטים
        </div>
      </div>
    </div>
  ...
  // Back face content (detailed description)
  const backContent = (
    <div
      className="relative w-full h-full rounded-2xl p-5 overflow-y-auto"
      style={{
        background: `linear-gradient(145deg, ${cfg.color}10, #0a0a0e)`,
        border: `1px solid ${cfg.borderColor}`,
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="text-2xl">{cfg.badge}</div>
        <h3 className="text-base font-black" style={{ color: cfg.color }}>{pkg.name}</h3>
      </div>
      
      <div className="text-white/80 text-[11px] leading-relaxed space-y-2 h-[calc(100%-80px)] overflow-y-auto pr-1 custom-scrollbar">
        {packageDetails[pkg.id]?.content.split('\n').map((line, i) => {
          if (line.includes('**')) {
            return <p key={i} className="font-bold text-white mt-2">{line.replace(/\*\*/g, '')}</p>;
          }
          if (line.trim().startsWith('•')) {
            return <p key={i} className="flex items-start gap-1 text-white/70">
      ...
      <div className="absolute bottom-3 left-0 right-0 flex justify-center">
        <button
          ...
          className="px-3 py-1.5 rounded-full text-[9px] font-black bg-white/10 border border-white/20 hover:bg-white/20 transition-all"
        >
          חזור לתפריט
        </button>
      </div>
  ...
  const frontContent = (
    ...
    >
      ...
      <div className="relative z-10 p-5 space-y-4 flex-grow flex flex-col">
        ...
        <div className="text-center text-[8px] text-amber-300/50 mt-1">
          👆 לחץ על כרטיס לפרטים
        </div>
      </div>
    </div>
  ...
  const backContent = (
    ...
    >
      ...
      <div className="absolute bottom-3 left-0 right-0 flex justify-center">
        <button
          onClick={(e) => { e.stopPropagation(); setIsFlipped(false); }}
          className="px-3 py-1.5 rounded-full text-[9px] font-black bg-amber-500/20 border border-amber-500/30 hover:bg-amber-500/30 transition-all text-amber-300"
        >
          חזור לתפריט
        </button>
      </div>
  ...
  const frontContent = (
    ...
    >
      ...
      <div className="relative z-10 p-5 space-y-4 flex-grow flex flex-col">
        ...
        <div className="text-center text-[8px] text-purple-300/30">
          👆 לחץ על כרטיס לפרטים
        </div>
      </div>
    </div>
  ...
  const backContent = (
    ...
    >
      ...
      <div className="absolute bottom-3 left-0 right-0 flex justify-center">
        <button
          onClick={(e) => { e.stopPropagation(); setIsFlipped(false); }}
          className="px-3 py-1.5 rounded-full text-[9px] font-black bg-purple-500/20 border border-purple-500/30 hover:bg-purple-500/30 transition-all text-purple-300"
        >
          חזור לתפריט
        </button>
      </div>
...
const EquipmentPackageCard = ({ pkg, onSelect }: { pkg: Package, onSelect: (p: Package) => void }) => {
  ...
  const frontContent = (
    ...
    >
      ...
      <div className="text-center text-[8px] text-white/30 mt-2">
        👆 לחץ על כרטיס לפרטים
      </div>
    </div>
  );
    ...
    <div
      ...
      style={{
        background: isHeavy 
          ...
          : 'linear-gradient(145deg, #1a1a1f, #0a0a0e)',
        border: isHeavy ? '1px solid rgba(234,88,12,0.3)' : '1px solid rgba(100,116,139,0.3)',
        boxShadow: isHeavy ? '0 0 30px rgba(234,88,12,0.15)' : '0 0 30px rgba(100,116,139,0.1)',
      }}
    >
      <div className="absolute top-0 left-0 right-0 h-1" style={{ 
        background: `linear-gradient(90deg, transparent, ${isHeavy ? '#ea580c' : '#94a3b8'}, transparent)` 
      }} />
      
      <div className="flex items-center gap-2 mb-3">
        {isHeavy ? <Truck size={18} className="text-orange-400" /> : <Wrench size={18} className="text-slate-400" />}
        <h3 className="text-lg font-black" style={{ color: isHeavy ? '#ea580c' : '#94a3b8' }}>{pkg.name}</h3>
      </div>
      ...
      <div className="text-white/80 text-[11px] leading-relaxed space-y-2 h-[calc(100%-80px)] overflow-y-auto pr-1 custom-scrollbar">
        {packageDetails[pkg.id]?.content.split('\n').map((line, i) => {
          ...
          if (line.trim().startsWith('•')) {
            return <p key={i} className="flex items-start gap-1 text-white/70">
              <span className={isHeavy ? 'text-orange-400' : 'text-slate-400'} style={{ marginTop: '2px' }}>•</span>
              <span>{line.replace('•', '')}</span>
            </p>;
          }
          if (line.trim() === '') return <div key={i} className="h-2" />;
      ...
      <div className="absolute bottom-3 left-0 right-0 flex justify-center">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-[9px] font-black border transition-all ${
            isHeavy 
              ? 'bg-orange-500/20 border-orange-500/30 hover:bg-orange-500/30 text-orange-300'
              : 'bg-slate-500/20 border-slate-500/30 hover:bg-slate-500/30 text-slate-300'
          }`}
        >
          חזור לתפריט
        </button>
  ...
  const frontContent = (
    ...
    >
      ...
      <div className="relative z-10 p-5 h-full flex flex-col">
        ...
        <div className="text-center text-[8px] text-blue-300/50 mt-2">
          👆 לחץ על כרטיס לפרטים
        </div>
      </div>
    </div>
  ...
  const backContent = (
    ...
    >
      ...
      <div className="absolute bottom-3 left-0 right-0 flex justify-center">
        <button
          onClick={(e) => { e.stopPropagation(); setIsFlipped(false); }}
          className="px-3 py-1.5 rounded-full text-[9px] font-black bg-blue-500/20 border border-blue-500/30 hover:bg-blue-500/30 transition-all text-blue-300"
        >
          חזור לתפריט
        </button>
      </div>
...
const CarDetailsForm = ({ 
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onNext}
        className="w-full py-3 rounded-xl font-black text-sm shadow-xl transition-all mt-2"
        style={{
      ...
      <main className="pt-24 px-3 max-w-6xl mx-auto">
        <AnimatePresence mode="wait">
          {view === 'home' && (
            ...
            >
              {/* ============================================================
                  HERO SECTION - Professional Animated Background (no video)
                  ============================================================ */}
              <section className="relative h-screen min-h-[600px] flex items-center justify-center overflow-hidden">
                ...
                <div className="absolute inset-0">
                  ...
                  {/* Animated mesh orbs */}
                  <div className="hero-orb-1 absolute top-[-10%] right-[-5%] w-[55%] h-[55%] rounded-full opacity-20"
                    style={{ background: 'radial-gradient(circle, #c8102e 0%, transparent 65%)' }} />
                  <div className="hero-orb-2 absolute bottom-[-15%] left-[-10%] w-[60%] h-[60%] rounded-full opacity-15"
                    style={{ background: 'radial-gradient(circle, #c8102e 0%, transparent 60%)' }} />
                  <div className="hero-orb-3 absolute top-[30%] left-[30%] w-[40%] h-[40%] rounded-full opacity-10"
                    style={{ background: 'radial-gradient(circle, #ff4560 0%, transparent 55%)' }} />

                  {/* Moving grid */}
                  <div className="absolute inset-0 overflow-hidden opacity-[0.04]">
                    <div className="grid-move absolute inset-0" style={{
                  ...
                  <div className="scan-line absolute left-0 right-0 h-[2px] pointer-events-none"
                    style={{ background: 'linear-gradient(90deg, transparent, rgba(200,16,46,0.4), transparent)' }} />

                  {/* Floating particles */}
                  {[...Array(8)].map((_, i) => (
                    <div
                      key={i}
                      className="absolute w-1 h-1 rounded-full bg-brand-red/40"
                      style={{
                        left: `${10 + i * 12}%`,
                        bottom: `${15 + (i % 3) * 10}%`,
                ...
                >
                  <motion.div
                    ...
                    className="inline-flex items-center gap-2 bg-white/6 backdrop-blur-md border border-brand-red/20 rounded-full px-4 py-2 mb-6"
                  >
                    <span className="w-1.5 h-1.5 rounded-full bg-brand-red animate-pulse" />
                    <span className="text-xs font-black tracking-wider text-white/80">הדרך המהירה ביותר למכור רכב</span>
                  </motion.div>
                  ...
                  >
                    מוכרים רכב? <br />
                    <span className="text-brand-red" style={{ textShadow: '0 0 40px rgba(200,16,46,0.4)' }}>
                  ...
                  >
                    <motion.button 
                      ...
                      className="px-8 py-4 rounded-xl font-black text-lg transition-all relative overflow-hidden group"
                      style={{
                        background: 'linear-gradient(135deg, #c8102e, #a50d25)',
                        boxShadow: '0 15px 40px -10px rgba(200,16,46,0.5)'
                      }}
                    >
                    ...
                    <button 
                      ...
                      className="px-8 py-4 bg-white/6 backdrop-blur-md rounded-xl font-black text-lg border border-white/12 hover:bg-white/10 hover:border-white/20 transition-all flex items-center gap-2"
                    >
                      <Search size={20} />
                      בדוק סטטוס
                    </button>
              ...
              <section id="packages" className="space-y-12">
                ...
                {/* Regular packages - FIXED: Now in a proper grid with flip cards */}
                <div>
                  <motion.div 
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {packages.map(pkg => (
                      <div key={pkg.id} className="h-[420px]">
                        <PackageCard 
                          pkg={pkg} 
                          lang={lang} 
                          onSelect={(p) => {
