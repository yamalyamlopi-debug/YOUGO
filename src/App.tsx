    ...
    <div className="min-h-screen pb-20">
      ...
      >
        {modalContent?.content}
                      ...
                      <Search size={20} />בדוק סטטוס
                    </button>
                  </motion.div>
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}
                    className="flex items-center justify-center gap-6 flex-wrap">
                    {[
                      { icon: <ShieldCheck size={16} />, label: 'תשלום מאובטח' },
                      { icon: <Clock size={16} />, label: 'פרסום תוך 24 שעות' },
                      { icon: <Users size={16} />, label: '50K+ עוקבים' },
                    ].map((item, i) => (
              ...
              <section id="how-it-works" className="space-y-10">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  {[
                    { step: '01', title: 'בחירת חבילה', desc: 'בוחרים את חבילת הפרסום המתאימה', icon: <LayoutDashboard size={24} />, color: 'from-blue-500 to-cyan-500' },
                    { step: '02', title: 'הזנת פרטים', desc: 'ממלאים פרטי הרכב ומעלים אישור תשלום', icon: <FileText size={24} />, color: 'from-brand-red to-red-600' },
                    { step: '03', title: 'פרסום וחשיפה', desc: 'הצוות שלנו מעצב ומפרסם מודעה מקצועית', icon: <Send size={24} />, color: 'from-green-500 to-emerald-500' },
                  ].map((item, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} whileHover={{ y: -5 }} className="relative group">
                      ...
                      <div className="relative p-5 text-center">
                        <div className={`w-14 h-14 mx-auto mb-4 rounded-xl bg-gradient-to-br ${item.color} flex items-center justify-center shadow-lg`}>{item.icon}</div>
              ...
              <section id="packages" className="space-y-12">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center space-y-2">
                  ...
                  <p className="text-white/50 text-xs md:text-sm">לחץ על כל כרטיס לפרטים נוספים 👆</p>
                </motion.div>

                {/* Regular packages — 3 columns side by side */}
                <div>
                  <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center space-y-2 mb-5">
                    ...
                    <h3 className="text-xl md:text-2xl font-black">מוכרים <span className="text-brand-red">רכב פרטי?</span></h3>
                  </motion.div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    {packages.map((pkg, i) => (
                      <motion.div key={pkg.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
                        <PackageCard pkg={pkg} lang={lang} onSelect={(p) => { setSelectedPackage(p); setView('booking'); setBookingStep(1); }} onShowDetails={handleShowPackageDetails} />
                      </motion.div>
                    ))}
                  </div>
                </div>

                {/* Premium: VIP + DUO side by side */}
                <div>
                  <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center space-y-2 mb-5">
                    ...
                    <h3 className="text-xl md:text-2xl font-black">מחפשים <span className="text-amber-400">יחס VIP?</span></h3>
                  </motion.div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.05 }}>
                      <VIPPackageCard pkg={vipPackage} lang={lang} onSelect={(p) => { setSelectedPackage(p); setView('booking'); setBookingStep(1); }} onShowDetails={handleShowPackageDetails} />
                    </motion.div>
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.12 }}>
                      <DuoDealPackageCard pkg={duoPackage} onSelect={(p) => { setSelectedPackage(p); setView('booking'); setBookingStep(1); }} onShowDetails={handleShowPackageDetails} />
                    </motion.div>
                  </div>
                </div>
                ...
                {/* Business */}
                <div>
                  <BusinessPackageCard pkg={businessPackage} onSelect={(p) => { setSelectedPackage(p); setView('booking'); setBookingStep(1); }} onShowDetails={handleShowPackageDetails} />
                </div>
                ...
                <div>
                  <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center space-y-2 mb-5">
                    ...
                    <h3 className="text-xl md:text-2xl font-black">מוכרים <span className="text-orange-400">ציוד מקצועי?</span></h3>
                  </motion.div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-3xl mx-auto">
                    {equipmentPackages.map((pkg, i) => (
                      <motion.div key={pkg.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
                        <EquipmentPackageCard pkg={pkg} onSelect={(p) => { setSelectedPackage(p); setView('booking'); setBookingStep(1); }} onShowDetails={handleShowPackageDetails} />
                      </motion.div>
                    ))}
                  </div>
                </div>
              ...
              <section id="why-us" className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[
                    { icon: <Users size={24} />, title: 'קהל איכותי', desc: '50,000+ עוקבים פעילים', stat: '50K+', color: 'from-blue-500 to-cyan-500' },
                    { icon: <Zap size={24} />, title: 'מהירות מכירה', desc: 'זמן ממוצע של 48 שעות', stat: '48h', color: 'from-brand-red to-red-600' },
                    { icon: <TrendingUp size={24} />, title: 'אחוזי הצלחה', desc: '98% מהלקוחות מרוצים', stat: '98%', color: 'from-green-500 to-emerald-500' },
                  ].map((item, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} whileHover={{ y: -5 }}
                      ...
                      <div className={`w-12 h-12 mb-3 rounded-xl bg-gradient-to-br ${item.color} flex items-center justify-center shadow-lg`}>{item.icon}</div>
              ...
              <section id="faq" className="max-w-3xl mx-auto space-y-8">
                    ...
                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                      ...
                      <button onClick={() => setActiveFaq(activeFaq === i ? null : i)}
                        ...
                        <div className="flex items-center gap-3 flex-1">
                          <div className="w-6 h-6 rounded-lg bg-brand-red/10 flex items-center justify-center text-brand-red font-black text-xs">{i + 1}</div>
                      ...
                      <AnimatePresence>
                        {activeFaq === i && (
                          <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                            <div className="px-4 pb-3 pr-12 text-white/60 text-xs leading-relaxed">{item.a}</div>
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </motion.div>
                ...
                {!showAllFaqs && t.faqs.length > 3 && (
                  <div className="text-center">
                    <button onClick={() => setShowAllFaqs(true)} className="px-5 py-2 bg-white/5 rounded-lg font-black text-xs border border-white/10 hover:bg-white/10 transition-all">
                      הצג את כל השאלות
                    </button>
                  </div>
                )}
              ...
              <footer className="pb-8">
                <div className="relative rounded-2xl overflow-hidden border border-white/8"
                  ...
                  <div className="relative z-10 p-8 space-y-8">
                    <div className="text-center space-y-3">
                      <div className="flex items-center justify-center gap-3">
                        <div className="p-2.5 bg-gradient-to-br from-brand-red to-red-700 rounded-xl shadow-xl shadow-brand-red/30"><Car size={20} className="text-white" /></div>
                        <div>
                          <div className="text-2xl font-black tracking-tighter"><span className="text-brand-red">YOUGO</span> <span className="text-white">ISRAEL</span></div>
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                      {[
                        { icon: <FileText size={12} />, label: 'תקנון', onClick: () => setModalContent({ title: 'תקנון שימוש', content: 'YOUGO ISRAEL - תקנון שימוש מלא זמין בפנייה לשירות הלקוחות.' }) },
                        { icon: <Lock size={12} />, label: 'פרטיות', onClick: () => setModalContent({ title: 'מדיניות פרטיות', content: 'YOUGO ISRAEL מכבדת את פרטיות המשתמשים. כל מידע שנמסר משמש לצורך מתן השירות בלבד.' }) },
                        { icon: <Info size={12} />, label: 'מי אנחנו', onClick: () => setModalContent({ title: 'אודות YOUGO ISRAEL', content: 'YOUGO ISRAEL היא פלטפורמת השיווק הדיגיטלי המובילה בישראל למכירת רכבים. 50,000+ עוקבים פעילים.' }) },
                        { icon: <LayoutDashboard size={12} />, label: 'ניהול', onClick: () => setView('admin-login') },
                      ].map((link, i) => (
                        <motion.button key={i} whileHover={{ y: -2 }} whileTap={{ scale: 0.95 }} onClick={link.onClick}
                    ...
                    <div className="text-center pt-2 border-t border-white/5">
                      <div className="text-white/15 text-[9px] font-bold tracking-wider">© {new Date().getFullYear()} YOUGO ISRAEL LTD · כל הזכויות שמורות</div>
                    </div>
                  </div>
                </div>
              </footer>
          ...
          {/* BOOKING */}
          {view === 'booking' && (
            <motion.div initial={{ opacity: 0, y: -60 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} className="max-w-2xl mx-auto space-y-6">
              <div className="flex items-center gap-3">
                <motion.button whileHover={{ x: -3 }} whileTap={{ scale: 0.96 }} onClick={() => setView('home')}
                  ...
                  <ArrowLeft size={15} /><span>חזרה לחבילות</span>
                </motion.button>
              </div>
              <div className="flex items-center justify-center gap-2 mb-4">
                {[1, 2].map(step => (
                  <React.Fragment key={step}>
                    <div className={`flex items-center gap-1 ${bookingStep >= step ? 'text-brand-red' : 'text-white/30'}`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-xs border-2 ${bookingStep >= step ? 'border-brand-red bg-brand-red/10' : 'border-white/20'}`}>{step}</div>
                      <span className="text-xs font-black">{step === 1 ? 'פרטי רכב' : 'תשלום'}</span>
                    </div>
                    {step === 1 && <div className={`w-12 h-px ${bookingStep >= 2 ? 'bg-brand-red' : 'bg-white/10'}`} />}
                  </React.Fragment>
                ))}
              </div>
          ...
          {/* SUCCESS */}
          {view === 'success' && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md mx-auto text-center space-y-6 py-12">
              ...
              <div className="space-y-2">
                ...
                <p className="text-white/60 text-sm">מספר הזמנה: <span className="text-brand-red font-black">#{orderId}</span></p>
              </div>
              <div className="glass-card p-5 space-y-4">
                ...
                <div className="space-y-2 text-right">
                  {['הודעת וואטסאפ נשלחה למנהל המערכת', 'הצוות שלנו יבדוק את פרטי ההזמנה תוך שעה', 'נחזור אליך עם אישור סופי'].map((text, i) => (
                    <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.1 }} className="flex items-center gap-2 text-xs">
                      <div className="w-5 h-5 rounded-full bg-green-500/20 flex items-center justify-center"><Check size={10} className="text-green-400" /></div>
              ...
              <div className="flex gap-2">
                <motion.button whileTap={{ scale: 0.96 }} onClick={() => setView('home')}
                  ...
                  <ArrowLeft size={13} />חזרה לדף הבית
                </motion.button>
                <motion.button whileTap={{ scale: 0.96 }} onClick={() => setView('check-status')}
          ...
          {view === 'admin-login' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-sm mx-auto py-12">
              <div className="glass-card p-6 space-y-5">
                <div className="text-center">
                  <div className="w-16 h-16 mx-auto mb-3 bg-gradient-to-br from-brand-red to-red-600 rounded-xl flex items-center justify-center shadow-lg"><Lock size={24} className="text-white" /></div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-4 mt-4">
                    <button onClick={() => setAdminTab('orders')} className={`px-6 py-2 rounded-full font-black text-sm transition-all ${adminTab === 'orders' ? 'bg-brand-red text-white' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}>הזמנות</button>
                    <button onClick={() => setAdminTab('settings')} className={`px-6 py-2 rounded-full font-black text-sm transition-all ${adminTab === 'settings' ? 'bg-brand-red text-white' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}>הגדרות אתר</button>
                  </div>
                </div>
                {adminTab === 'orders' && (
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={fetchOrders} className="btn-primary py-3 px-6 flex items-center gap-2"><ArrowLeft size={20} className="rotate-90" />רענן נתונים</button>
                  </div>
                )}
              </div>
              {adminTab === 'orders' ? (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    {[
                      ...
                      <motion.div key={i} whileHover={{ y: -5 }} className="bg-white rounded-3xl p-6 shadow-xl border border-white/10 flex flex-col gap-4">
                        <div className="flex items-center justify-between">
                            ...
                            <tr key={order.id} className="hover:bg-white/5 transition-colors">
                              ...
                              <td className="p-6">
                                <div className={`inline-flex items-center gap-2 text-[10px] font-bold px-3 py-1.5 rounded-full border ${order.status === 'Published' ? 'bg-green-500/10 text-green-500 border-green-500/20' : order.status === 'Rejected' ? 'bg-red-500/10 text-red-500 border-red-500/20' : order.status === 'Payment Verified' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' : 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20'}`}>
                                  <div className={`w-1.5 h-1.5 rounded-full ${order.status === 'Published' ? 'bg-green-500' : order.status === 'Rejected' ? 'bg-red-500' : order.status === 'Payment Verified' ? 'bg-blue-500' : 'bg-yellow-500'}`} />{order.status}
                                </div>
                              </td>
                              <td className="p-6">
                                <div className="flex items-center justify-center gap-3">
                                  <button onClick={() => alert(`פרטי הזמנה ${order.id}:\nמחיר: ${order.car_price}\nמיקום: ${order.location}`)} className="p-2 bg-white/5 hover:bg-white/10 rounded-lg transition-colors"><Eye size={18} /></button>
                                  <select className="bg-dark-card border border-white/10 text-xs rounded-lg p-2 focus:border-brand-red outline-none" value={order.status} onChange={(e) => updateOrderStatus(order.id, e.target.value)}>
                                    <option value="Pending Review">ממתין</option>
                                    <option value="Payment Verified">תשלום אושר</option>
                                    <option value="Published">פורסם</option>
                                    <option value="Rejected">נדחה</option>
                ...
                <div className="glass-card p-8 max-w-2xl space-y-8">
                  ...
                  <div className="space-y-6">
                    {[
                      { label: 'כמות עוקבים', key: 'followers_count' },
      ...
      <ChangePackageModal
        ...
        onSelect={(p) => {
          setSelectedPackage(p);
          setFormData({ fullName: formData.fullName, phone: formData.phone, model: '', year: '', mileage: '', price: '', registration: '', testUntil: '', location: formData.location, paymentProof: '', carImages: [], model2: '', year2: '', mileage2: '', price2: '', registration2: '', testUntil2: '', agencyName: '', monthlyCars: '', agencyDetails: '' });
        }}
        lang={lang}
      />
    </div>
